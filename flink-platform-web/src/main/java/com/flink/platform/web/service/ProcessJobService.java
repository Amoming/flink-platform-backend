...
public class ProcessJobService {
    ...
    @Autowired
    private List<CommandBuilder> jobCommandBuilders; // 所有的实现类都会进入到List中 后续通过stream筛选

    @Autowired
    private List<CommandExecutor> jobCommandExecutors;
...
//    }
    ...
    public void processJob(final long jobRunId) throws Exception {
        ...
        try {
            ...
            String version = jobRunInfo.getVersion(); // 获取版本

            // step 3: build job command, create a SqlContext if needed
            jobCommand = jobCommandBuilders.stream()
                    .filter(builder -> builder.isSupported(jobType, version)) // 是否包含FLINK 是否存在该版本
                    .findFirst()
